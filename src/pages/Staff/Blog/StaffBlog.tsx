  ...
  return (
    <>
      {loading ? (
        ...
        <motion.div className="p-8 space-y-6 bg-gray-100 overflow-x-hidden" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            {blogs.map((blog) => (
              <motion.div
                key={blog.id}
                className={`flex flex-col bg-white p-4 rounded shadow space-y-2 ${blog.isActived ? "" : "hidden"}`}
